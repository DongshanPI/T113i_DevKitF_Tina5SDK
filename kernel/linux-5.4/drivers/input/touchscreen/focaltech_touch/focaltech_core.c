...
static int fts_read_touchdata(struct fts_ts_data *data)
{
    ...
    static bool fts_i2c_permanently_failed = false;

    if (fts_i2c_permanently_failed) {
        return -EIO; // 已永久失败，直接返回
    }
    ...
    if (ret < 0) {
        if (i2c_error_count < 1 || (i2c_error_count % 100 == 0)) {
            // FTS_ERROR("read touchdata failed, ret:%d", ret);
        }
        i2c_error_count++;
        if (i2c_error_count > 5) {
            fts_i2c_permanently_failed = true;
            // printk(KERN_ERR "FTS: I2C failed over 5 times, stop further access!\n");
        }
        return ret;
    } else {
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    if (pdata->have_key) {
        ...
        if (!ret && pdata->key_number > FTS_MAX_KEYS)
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (ret) {
        // FTS_ERROR("not focal IC, but continue init for debug/compatibility");
        ret = 0; // 不再退出，继续初始化
    }
